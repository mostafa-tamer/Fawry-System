...
public class NormalUserHomeBoundary extends HomeBoundary {
    ...
    private boolean userSelectionsFactory(int selection) throws Exception {
        switch (selection) {
            ...
            case 6 -> {
                throw new CustomException("Logging out...");
            }
            default -> {
    ...
    private void requestRefund() {
        ...
        try {
            myTransactions = transactions.showMyTransactions(user.getEmail());
        } catch (SQLException e) {
            e.printStackTrace();
            return;
        }
        if (myTransactions.size() > 0) {
            displayAllTransactions(myTransactions);
            do {
                ...
                for (var transaction : myTransactions)
                    if (id == transaction.getId()) {
                        try {
                            if (transactions.requestRefund(id))
                                printDecorated("Refund has been requested Successfully", '-');
                            else
                                System.out.println("couldn't request refund, please choose another transaction");
                            return;
                        } catch (SQLException e) {
                            e.printStackTrace();
                        }
            ...
            System.out.println("There's no transactions to refund");
    }
